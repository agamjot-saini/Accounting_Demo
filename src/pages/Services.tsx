...
import { User, Building2, BookOpen, Receipt, Briefcase, DollarSign, LineChart, ArrowRight, Video as LucideIcon } from 'lucide-react';
  ...
  return (
    <div className={`flex flex-col ${reversed ? 'lg:flex-row-reverse' : 'lg:flex-row'} gap-10 lg:gap-16 items-center`}>
      <div className="lg:w-1/2">
        ...
        <h2 className="font-heading text-3xl md:text-4xl font-extrabold tracking-tight text-[#0F172A] mb-4">
          {title}
        </h2>
        {description.map((para, i) => (
          <p key={i} className="text-[#334155] leading-7 mb-4">{para}</p>
        ))}
        <h3 className="font-heading font-bold text-sm text-[#0F172A] uppercase tracking-wider mb-3 mt-6">
          What's Included
        </h3>
        ...
        <Link
          to="/get-started"
          className="inline-flex items-center justify-center gap-2 rounded-xl text-white font-semibold font-heading px-6 py-3.5 min-h-[48px] text-[15px] shadow-[0_10px_24px_rgba(37,99,235,0.25)] hover:shadow-[0_14px_30px_rgba(37,99,235,0.30)] active:translate-y-[1px] transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#93C5FD] focus-visible:ring-offset-2"
      ...
      <div className="lg:w-1/2">
        <div className="w-full aspect-[4/3] rounded-2xl bg-[#EFF6FF] border border-[#DBEAFE] flex items-center justify-center">
  ...
  return (
    <>
      ...
      {services.map((service, index) => (
        <section
          key={service.title}
          className={`py-16 md:py-20 lg:py-28 ${index % 2 === 0 ? '' : 'bg-white'}`}
        >
          <Container>
